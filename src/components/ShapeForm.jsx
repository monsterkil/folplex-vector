export default function ShapeForm({ shape, setShape }) {
  ...
  const handleHolesChange = (field, value) => {
    ...
    if (field === 'count') {
      if (type === 'circle') numValue = Math.min(16, Math.max(1, numValue))
      else if (type === 'rectangle' || type === 'square') {
        const v = parseInt(numValue, 10) || 1
        numValue = [1, 2, 3, 4, 8].includes(v) ? v : (v > 4 ? 8 : Math.min(4, Math.max(1, v)))
      } else numValue = Math.min(4, Math.max(1, numValue))
    }
    setShape(prev => ({
      ...prev,
      ...
      {hasHolesOption && (
        <div className="space-y-2.5">
          ...
          {holes.enabled && (
            <>
              <div>
                <label className="input-label text-xs">{type === 'circle' ? 'Ilość (1–16)' : (type === 'rectangle' || type === 'square' ? 'Ilość (1, 2, 3, 4 lub 8)' : 'Ilość (1–4, np. 4 rogi)')}</label>
                <select
                  value={type === 'circle' ? (holes.count ?? 4) : (type === 'rectangle' || type === 'square' ? ([1, 2, 3, 4, 8].includes(holes.count) ? holes.count : 4) : Math.min(4, holes.count ?? 4))}
                  onChange={(e) => handleHolesChange('count', e.target.value)}
                  className="input-field"
                >
                  {(type === 'circle' ? Array.from({ length: 16 }, (_, i) => i + 1) : type === 'rectangle' || type === 'square' ? [1, 2, 3, 4, 8] : [1, 2, 3, 4]).map(n => (
